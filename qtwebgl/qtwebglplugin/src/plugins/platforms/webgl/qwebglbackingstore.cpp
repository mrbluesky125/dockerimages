...
QWebGLBackingStore::QWebGLBackingStore(QWindow *window)
...
QWebGLBackingStore::~QWebGLBackingStore()
{
    if (m_bsTexture) {
...
QPaintDevice *QWebGLBackingStore::paintDevice()
{
    return &m_image;
}

void QWebGLBackingStore::updateTexture()
{
    if (!m_bsTexture) {
...
void QWebGLBackingStore::flush(QWindow *window, const QRegion &region, const QPoint &offset)
{
...
void QWebGLBackingStore::composeAndFlush(QWindow *window, const QRegion &region, const QPoint &offset,
...
void QWebGLBackingStore::notifyComposited()
{
...
void QWebGLBackingStore::beginPaint(const QRegion &region)
{
    m_dirty |= region;
...
void QWebGLBackingStore::resize(const QSize &size, const QRegion &staticContents)
{
...
QImage QWebGLBackingStore::toImage() const
{
    return m_image;
}
